...
$type = query_val_by_key('class', 'tree', 'id', $ref);
$table = query_val_by_key('table_name', 'object_class', 'id', $type);
$field = query_val_by_key('table_field', 'object_property', 'id', $el);

$id = query_val_by_key('id', $table, 'id', $ref, FALSE);
if ($id == '')
{
        $query = "INSERT INTO `$table` (`id`) VALUES ('$ref')";
        $sql = mysql_query($query) or die(mysql_error());
}

$query = "UPDATE `$table` SET `$field` = '{$config['video_w']}x{$config['video_h']}' WHERE `id` = '$ref'";
$sql = mysql_query($query) or die(mysql_error());

// process image file

// copy uploaded image
$dest = '../storage/'.$ref.'_'.$el.'.flv';
unlink($dest);
copy($uploaded, $dest);
unlink($uploaded);
...
if (!$swf)
{
    print "
        ...
        top.ObjEditors[$ref].updateVideo($el);
        top.document.getElementById('id'+$ref+'element'+$el+'_w').value = '{$config['video_w']}';
        top.document.getElementById('id'+$ref+'element'+$el+'_h').value = '{$config['video_h']}';
        </script>
    ";
}
